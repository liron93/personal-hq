...
import hrQuestions from "./data/hr-questions.json";
...
const HR_BANK = [...hrQuestions, ...hrTerminology];

function TrackSwitch({ prefs, setPrefs }) {
  const track = normalizePrefs(prefs).track;
  return <div role="group" aria-label="מסלול תרגול" style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "8px 0" }}>{Object.entries(TRACKS).map(([id, label]) => <button key={id} type="button" aria-pressed={track === id} className={track === id ? styles.primary : styles.secondary} onClick={() => setPrefs(p => setTrack(p, id))}>{label}</button>)}</div>;
}

export function PracticeHub({ user, jobs, prefs, setPrefs }) {
  ...
  const track = normalizePrefs(prefs).track;
  const bank = questionsFor(track, { pm: questions, hr: HR_BANK });
  const categories = [...new Set(bank.map(item => item.category))];
  const visible = bank.filter(item => (category === "all" || item.category === category) && (difficulty === "all" || item.difficulty === difficulty));
  const submit = async event => {
    event.preventDefault();
    if (!answer.trim() || !selected) return;
    if (isHrQuestion(selected.id)) {
      // מסלול HR: הניסיון נשמר במרחב האישי (לא ב-career_answers, שדורש שאלה מזורעת ב-career_questions).
      setPrefs(p => addHrAttempt(p, selected.id, answer, { id: crypto.randomUUID(), now: Date.now() }));
      setMessage("התשובה נשמרה בהיסטוריה שלך.");
      return;
    }
  ...
  if (selected) return <section className={styles.panel}><div className={styles.sectionTitle}><div><p>{selected.category} · {difficultyLabels[selected.difficulty]}</p><h2>תרגול שאלה</h2></div><button className={styles.close} onClick={() => {setSelected(null);setAnswer("");setMessage("");}}>חזרה לבנק</button></div>
    ...
    {selected.question_type === "multiple_choice" ? <article className={styles.practiceQuestion}><ul>{(selected.options || []).map(opt => <li key={opt} style={{ fontWeight: opt === selected.correct_answer ? 700 : 400 }}>{opt}</li>)}</ul>{selected.explanation && <p className={styles.muted}>{selected.explanation}</p>}</article> : <form className={styles.form} onSubmit={submit}><label className={styles.field}><span>התשובה שלך</span><textarea rows="9" value={answer} onChange={event => setAnswer(event.target.value)} placeholder="כתוב/י תשובה כפי שהיית אומר/ת אותה בראיון" /></label>{message && <p className={styles.notice}>{message}</p>}<button className={styles.primary} disabled={saving}>{saving ? "שומר…" : "שמירת ניסיון"}</button></form>}
  </section>;
  return <section className={styles.panel}><div className={styles.sectionTitle}><div><p>{track === "hr" ? "בנק שאלות למנהלת משאבי אנוש" : "העברה מלאה של מאגר השאלות"}</p><h2>תרגול ושאלות</h2></div><span className={styles.active}>{bank.length} שאלות</span></div>
    <TrackSwitch prefs={prefs} setPrefs={setPrefs} />
    ...
    <div className={styles.questionGrid}>{visible.map(item => <button className={styles.questionCard} key={item.id} onClick={() => setSelected(item)}><span>{item.category} · {difficultyLabels[item.difficulty]}</span><strong>{item.question}</strong><small>{(item.frameworks || []).join(" · ") || (item.question_type === "multiple_choice" ? "שאלה אמריקאית" : "")}</small></button>)}</div>
  </section>;
}

export function HistoryHub({ user, prefs }) {
  const [remote, setRemote] = useState([]); const [loading, setLoading] = useState(true);
  useEffect(() => { let live=true; supabase.from("career_answers").select("id, question_id, answer_text, overall_score, created_at, career_questions(question, category)").eq("user_id",user.id).order("created_at",{ascending:false}).then(({data}) => { if(live){setRemote(data||[]);setLoading(false);} }); return () => {live=false}; },[user.id]);
  const answers = [...hrHistory(prefs, HR_BANK), ...remote].sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)));
...
export function LearningHub({ user, prefs }) {
  ...
  const track = normalizePrefs(prefs).track; const glossary = conceptsFor(track, { pm: concepts, hr: hrConcepts });
  const shown=glossary.filter(item=>(item.term+" "+item.category+" "+item.explanation).toLowerCase().includes(query.toLowerCase()));
  return <div className={styles.stack}><section className={styles.panel}><div className={styles.sectionTitle}><div><p>{track === "hr" ? "מילון מושגים למשאבי אנוש" : "מילון מקצועי שהועבר מ־Job-guide"}</p><h2>מושגים ולמידה</h2></div><span className={styles.active}>{glossary.length} מושגים</span></div><input className={styles.search} value={query} onChange={event=>setQuery(event.target.value)} placeholder="חיפוש מושג או מסגרת עבודה"/><div className={styles.concepts}>{shown.slice(0,24).map(item=><article className={styles.concept} key={item.term}><span>{item.category}</span><h3>{item.term}</h3><p>{item.explanation}</p></article>)}</div></section>
