...
export default function OriaControlRoom(){
 ...
 <article className={css.card}><div className={css.head}><h2>שלוש פעולות להיום</h2><span>{today.length}/3</span></div>{!today.length?<p className={css.empty}>בחר פעולה אחת. לא צריך לתכנן את כל החיים היום.</p>:today.slice(0,3).map(row)}</article>
 {today.length>3&&<article className={css.reliefCard}><Clock3 size={20}/><div><strong>שחרור עומס</strong><p>יש {today.length} פעולות להיום. העבר אחת למחר כדי להחזיר את היום לגודל שאפשר לבצע.</p></div></article>}</section>}
