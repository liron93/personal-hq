...
import { Check, ChevronLeft, Clock3, Plus, Radar, Trash2 } from "lucide-react";
...
const afterTomorrowISO=()=>{const d=new Date();d.setDate(d.getDate()+2);return new Date(d.getTime()-d.getTimezoneOffset()*60000).toISOString().slice(0,10)};
...
export default function OriaControlRoom(){
 ...
 const [tab,setTab]=useState("today"),[title,setTitle]=useState(""),[slot,setSlot]=useState("today"),[notice,setNotice]=useState("");
 ...
 const today=tasks.filter(x=>x.bucket==="today"&&!x.done),tomorrow=tasks.filter(x=>x.bucket==="tomorrow"&&!x.done),week=tasks.filter(x=>x.bucket==="week"&&!x.done);
 ...
 const add=()=>{if(!title.trim())return;const bucket=slot==="tomorrow"?"tomorrow":slot==="later"?"inbox":"today";mutate(xs=>[{id:uid(),title:title.trim(),bucket,due:slot==="tomorrow"?tomorrowISO():"",source:"אוריה",owner:"לירון",done:false},...xs]);setTitle("");tell("הפעולה נוספה.");};
 ...
 const row=(task)=><article key={task.id} className={css.commandRow}><button className={css.check} onClick={()=>done(task.id)} aria-label="סמן שבוצע"><Check size={15}/></button><div className={css.rowMain}><strong>{task.title}</strong><span><i>{source(task)}</i> · בעלים: {owner(task)}</span></div><button className={css.delete} onClick={()=>remove(task.id)} aria-label="מחיקה"><Trash2 size={17}/></button></article>;
 ...
 {tab==="today"&&<section className={css.stack}><section className={css.nowCard}><p>עכשיו</p>{now?<><h2>{now.title}</h2><span>מקור: {source(now)} · בעלים: {owner(now)}</span><button onClick={()=>done(now.id)}><Check size={17}/>סימון בוצע</button></>:<><h2>אין פעולה בוערת כרגע.</h2><span>זה זמן טוב להכניס דבר אחד שחשוב לך.</span></>}</section>
 <article className={css.card}><div className={css.head}><h2>הכנסה מהירה</h2><span>בלי תאריך ידני</span></div><textarea className={css.field} value={title} onChange={e=>setTitle(e.target.value)} placeholder="מה צריך לטפל בו?" /><div className={css.quickChoices}><button className={slot==="today"?css.chosen:""} onClick={()=>setSlot("today")}>היום</button><button className={slot==="tomorrow"?css.chosen:""} onClick={()=>setSlot("tomorrow")}>מחר</button><button className={slot==="later"?css.chosen:""} onClick={()=>setSlot("later")}>לא חשוב כרגע</button><button className={css.primary} onClick={add}><Plus size={18}/>הוספה</button></div></article>
 <article className={css.card}><div className={css.head}><div><h2>כל המשימות להיום</h2><p className={css.hint}>שלוש הראשונות הן נקודת הפתיחה — אבל שום משימה לא מוסתרת.</p></div><span>{today.length} משימות</span></div>{!today.length?<p className={css.empty}>בחר פעולה אחת. לא צריך לתכנן את כל החיים היום.</p>:today.map(row)}</article>
 {today.length>3&&<article className={css.reliefCard}><Clock3 size={20}/><div><strong>מבט ריאלי על היום</strong><p>יש {today.length} משימות, וכולן מוצגות כאן. מתחילים בשלוש הראשונות; את השאר אפשר להזיז למחר או להשאיר ברשימה.</p></div></article>}</section>}
 {tab==="soon"&&<section className={css.stack}><section className={css.hero+" "+css.light}><p>48 השעות הבאות</p><h2>מה מתקרב?</h2><span>רק מה שצריך לקבל מקום ביומן ובתשומת הלב שלך.</span></section><article className={css.card}><h2>מחר · {new Date(tomorrowISO()+"T12:00").toLocaleDateString("he-IL",{weekday:"long",day:"numeric",month:"long"})}</h2>{!tomorrow.length?<p className={css.empty}>אין פעולות מתוכננות למחר.</p>:tomorrow.map(row)}</article><article className={css.card}><h2>מחרתיים והלאה</h2>{!week.length?<p className={css.empty}>אין פעולה קרובה נוספת.</p>:week.map(row)}</article></section>}
