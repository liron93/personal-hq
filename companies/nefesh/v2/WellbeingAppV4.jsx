...
const TABS = [["today","היום"],["week","סדר היום"],["private","פרטי"]];
const loads = [["light","יש לי מקום"],["busy","די מלא"],["heavy","אין לי מקום"]];
...
export default function WellbeingAppV4() {
  ...
  return <main className={css.shell} dir="rtl">
    ...
    {tab==="today"&&<section className={css.stack}>
      ...
      <article className={css.card}><h2>כמה מקום יש לך היום?</h2><p className={css.muted}>לא מדד ולא אבחון — רק כדי שנבין כמה אפשר להכניס ליום הזה.</p><div className={css.choices}>{loads.map(([id,label])=><button key={id} className={today.load===id?css.selected:""} onClick={()=>{saveToday({load:id});tell("מצב העומס נשמר.");}}>{label}</button>)}</div>
      {today.load==="heavy"&&<div className={css.relief}><strong>בוא נוריד משהו מהכתפיים.</strong><p>בחר פעולה אחת, בלי להסביר ובלי לשפוט.</p><div className={css.reliefButtons}>{reliefs.map(([id,label])=><button key={id} className={today.reliefPlan===id?css.selected:""} onClick={()=>{saveToday({reliefPlan:id}); if(id==="ask")tell("כתוב לי כאן בצ׳אט מה אתה צריך, ונפרק את זה יחד."); tell("תוכנית ההקלה נשמרה.");}}>{label}</button>)}</div><input className={css.field} value={today.reliefNote||""} onChange={e=>saveToday({reliefNote:e.target.value})} placeholder="הערה קצרה, אם זה עוזר" /></div>}</article>
      ...
      <article className={css.card}><h2>במה תרצה שאעזור היום?</h2><p className={css.muted}>כתוב לי כאן בצ׳אט את הדבר שמסתובב לך בראש — אני אעזור לפרק, לתעדף ולבנות צעד הבא.</p><button className={css.primary} onClick={()=>tell("אני כאן. כתוב לי בצ׳אט מה הדבר הראשון שצריך סדר.")}>להתחיל עם אוריה</button></article>
    ...
    {tab==="support"&&false&&<section className={css.stack}>
