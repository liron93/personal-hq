...
function ProgramCard({ profile }) {
  const restricted = profile.safety && profile.safety !== "לא ידוע לי על מגבלה";
  const count = Number(profile.availability) || 2;
  const home = (Array.isArray(profile.place) ? profile.place : [profile.place]).some(place => place?.includes("בית"));
  const dislikes = Array.isArray(profile.dislikes) ? profile.dislikes : [profile.dislikes].filter(Boolean);
  const stepTargets = {"עד 3,000":"4,000","3,000–5,000":"6,000","5,000–7,500":"8,000","7,500–10,000":"10,000","מעל 10,000":"10,500","לא יודע/ת":"יעד לבחירה"};
  const upper = home ? ["סקוואט לכיסא — 2×8–12","שכיבות סמיכה על קיר / שולחן — 2×8–12","גשר ישבן — 2×10–12","חתירה עם גומייה או תיק — 2×8–12"] : ["לחיצת רגליים — 2×8–12","לחיצת חזה במכונה — 2×8–12","חתירה בישיבה — 2×8–12","פלאנק קצר — 2×20–30 שניות"];
  const move = dislikes.includes("ריצה") ? "הליכה בקצב נוח" : "הליכה מהירה או פעילות שאתה אוהב";
  if (restricted) return <section className={css.card+" "+css.programCard}><h3>לפני שבונים תוכנית</h3><p>סימנת שיש מגבלה או אי־ודאות. כדי לא לקחת סיכון, לא נוצרת כאן תוכנית תרגילים אישית. אפשר להמשיך בתיעוד, הליכה נוחה רק אם מתאים, ולבקש מאיש/ת מקצוע לבנות את התוכנית.</p></section>;
  return <section className={css.card+" "+css.programCard}><p className={css.eyebrow}>טיוטת תוכנית · 4 שבועות ראשונים</p><h3>הכיוון שלך: {Array.isArray(profile.goal) ? profile.goal.join(" · ") : profile.goal}</h3><p className={css.subtle}>המטרה היא להתחיל בקצב שניתן להתמיד בו, ואז לעדכן לפי התיעוד שלך.</p><div className={css.programStats}><span><strong>{count}</strong> אימונים בשבוע</span><span><strong>{stepTargets[profile.stepsBaseline] || "יעד אישי"}</strong> צעדים ביום</span><span><strong>{profile.sessionLength || "20–30 דקות"}</strong> לאימון</span></div><div className={css.workoutBlock}><h4>אימון כוח A · {profile.sessionLength || "20–30 דקות"}</h4>{upper.map(item => <p key={item}>• {item}</p>)}</div><div className={css.workoutBlock}><h4>תנועה בין אימונים</h4><p>• {move} במשך הזמן שבחרת, בקצב שמאפשר לדבר.</p><p>• הוספת צעדים הדרגתית עד ליעד היומי שהוגדר כאן.</p></div><p className={css.safetyNote}>זהו מבנה התחלתי, לא אבחון או טיפול. אם יש כאב חד, סחרחורת או תחושה חריגה — עוצרים ופונים לאיש/ת מקצוע.</p></section>;
}
