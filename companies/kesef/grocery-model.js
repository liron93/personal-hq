// לוגיקה טהורה של "סופר" (רשימת קניות משותפת) — קטגוריות, זיהוי אוטומטי, מיון/קיבוץ לפי מסלול קנייה,
// מעבר בין "צריך לקנות" להיסטוריית רכישות, וחישובי חיסכון/תקציב/מגמה/התראות.
// קובץ טהור בכוונה (בלי React, בלי Supabase, בלי window, ובלי alias imports של "@/") כדי שיהיה
// אפשר לייבא אותו ישירות תחת node:test (`node --test`) בלי טוען מודולים מיוחד.
// הנתונים חיים כשדה `grocery` בתוך hq:kesef:v1 (ראה model.js), ולכן יורשים אוטומטית את היכולת
// finance.dashboard_budget.read/write הקיימת (owner מלא, ליאור B+ עריכה, שקד בלי גישה בכלל — ראה lib/authz/capabilities.js).

// זהה ל-uid() ב-lib/format.js, משוכפל כאן בכוונה כדי שהמודול יישאר טהור וללא import של "@/".
const uid = () => Math.random().toString(36).slice(2, 10);

// סדר הקטגוריות = גם רשימת הקטגוריות הסופית וגם סדר מסלול הקנייה (superset של שתי ההערות של עמית,
// עם "אחר" תמיד אחרון כפי שהוגדר בהערה השנייה).
...
// מילון ברירת מחדל לזיהוי קטגוריה לפי מילת מפתח בשם הפריט. ניתן לעריכה על ידי המשתמש (נשמר ב-state.categoryDict),
// ולכן זו רק ברירת המחדל ההתחלתית — לא מקור אמת קבוע.
...
/** מבטיח שדה grocery תקין גם אם חסר בנתונים ישנים (תאימות אחורה, בלי לגעת בשאר ה-state). */
export function ensureGroceryState(d) {
  if (!d) return d;
  if (d.grocery && Array.isArray(d.grocery.items) && Array.isArray(d.grocery.history)) return d;
  return { ...d, grocery: { ...createGroceryState(), ...(d.grocery || {}) } };
}
...
export function groceryAlerts(state, { referenceMonth = nowMonth(), trendThresholdPercent = 20 } = {}) {
  ...
  if (bva.over) {
    alerts.push({
      ...
      message: `חריגה מהתקציב החודשי לסופר: הוצאת ${ils(bva.spend.total)} מתוך תקציב של ${ils(bva.budget)} החודש (${referenceMonth}).`,
    });
  }
