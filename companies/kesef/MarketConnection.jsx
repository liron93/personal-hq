...
export default function MarketConnection({result,setResult,symbol,setSymbol}) {
  ...
  useEffect(()=>{check();},[]);
  async function connect(e){e.preventDefault();setBusy(true);setError('');try{setResult(await call('POST',{symbol}));}catch(e){setError(e.message);}finally{setBusy(false);}}
  ...
  return <section className={s.panel} dir="rtl"><header><div><p className={s.eyebrow}>US MARKET / EODHD</p><h2>חיבור נתוני שוק</h2><p>מחירי סוף יום והיסטוריה של עד שנה — מתוך האפליקציה.</p></div><span className={s.badge}>{result?'התקבלו נתונים מהספק':status?.configured?'מוגדר · טרם אומת':busy?'בודק…':'החיבור אינו מוכן'}</span></header>
    ...
    <p className={s.footnote}>התוצאה האחרונה נשמרת במעבר בין מחלקות בחברה, ללא קריאת ספק נוספת. רענון הדף או יציאה מהחברה ינקו אותה; זו אינה שמירה קבועה בתיק האישי. תאריך השליפה מוצג לצד הנתונים.</p>
