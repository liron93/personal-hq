    ...
    <div style={{ display: "grid", gap: 12 }}>
      ...
      <div>
        ...
        {(draft.lines || []).map((l, i) => { const setLine = (k, v) => setDraft(d => ({ ...d, lines: d.lines.map((q, j) => j === i ? { ...q, [k]: v } : q) })); return <div key={l.id} className="renovation-line-edit" style={{ display: "grid", gap: 8, padding: 10, marginTop: 8, border: "1px solid #DCE3DE", borderRadius: 10, background: "#FAFCFB" }}>
          <div data-form-grid style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><Field label="שם"><input aria-label="שם שורת פירוט" value={l.name} onChange={e => setLine("name", e.target.value)} style={input} /></Field><Field label="יצרן ודגם"><input aria-label="יצרן ודגם" value={l.model || ""} onChange={e => setLine("model", e.target.value)} style={input} /></Field></div>
          <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 1fr", gap: 8 }}><Field label="כמות"><input dir="ltr" inputMode="numeric" aria-label="כמות" value={l.qty} onChange={e => setLine("qty", Number(e.target.value))} style={input} /></Field><Field label="מחיר ליחידה"><input dir="ltr" inputMode="numeric" aria-label="מחיר ליחידה" value={l.price} onChange={e => setLine("price", Number(e.target.value))} style={input} /></Field><Field label="מצב"><select aria-label="מצב שורת פירוט" value={l.status || "לבדיקה"} onChange={e => setLine("status", e.target.value)} style={input}>{LINE_STATUSES.map(x => <option key={x}>{x}</option>)}</select></Field></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 44px", gap: 8, alignItems: "end" }}><Field label="הערה"><input aria-label="הערה לשורת פירוט" value={l.note || ""} onChange={e => setLine("note", e.target.value)} style={input} /></Field><button type="button" aria-label="הסרת שורת פירוט" onClick={() => setDraft(d => ({ ...d, lines: d.lines.filter((_, j) => j !== i) }))} style={{ minHeight: 44, border: 0, background: "transparent", color: "#B53A31" }}><Trash2 size={18} /></button></div>
        </div>; })}
...
function Items({ data, edit, add, remove }) {
  const [room, setRoom] = useState(""); const [status, setStatus] = useState(""); const [q, setQ] = useState(""); const [view, setView] = useState("grouped"); const [open, setOpen] = useState({});
  ...
  const visible = data.items.filter(i => (!room || i.room === room) && (!status || i.status === status) && (matches(i) || (i.lines || []).some(l => matches(i, l))));
  const flat = data.items.filter(i => (!room || i.room === room) && (!status || i.status === status)).flatMap(i => (i.lines || []).length ? i.lines.map(l => ({ item: i, line: l })) : [{ item: i, line: null }]).filter(({ item, line }) => matches(item, line));
  ...
  return <div style={{ display: "grid", gap: 12 }}>
    ...
    <div className="renovation-filterbar"><select aria-label="סינון לפי חדר" value={room} onChange={e => setRoom(e.target.value)}><option value="">כל החדרים</option>{rooms.map(x => <option key={x}>{x}</option>)}</select><select aria-label="סינון לפי מצב" value={status} onChange={e => setStatus(e.target.value)}><option value="">כל המצבים</option>{["לבחירה","בהצעת מחיר","הוזמן","בהמתנה לאספקה","בהתקנה","הושלם","מעוכב"].map(x => <option key={x}>{x}</option>)}</select></div>
