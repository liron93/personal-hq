...
* ליאור (חבילה B+, `partner_full_finance_edit`): רואה **ועורכת** בית חדש (כולל קבצי בית חדש) ואת כל הכספים **כולל תנועות** (עריכת תקציב ותנועות). נתוני הליבה (`hq:core:v1`) בקריאה בלבד (אין `core.write` בחבילה). לא רואה נפשי, בריאות, קריירה של לירון. הקריירה שלה ריקה.
* שקד: רואה ועורכת **רק** בית חדש (כולל מסמכים). לא רואה HQ, כספים, נפשי, בריאות, קריירה.
