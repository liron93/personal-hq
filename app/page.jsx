...
function CEO({ access }) {
  ...
  const who = isMember ? "" : ", לירון";
  const greeting = today.getHours() < 12 ? `בוקר טוב${who}` : today.getHours() < 18 ? `צהריים טובים${who}` : `ערב טוב${who}`;
  const companies = useMemo(() => visibleList.map(c => ({ ...c, summary: summaries[c.slug]?.summary, data: summaries[c.slug]?.data, ...meta[c.slug] })), [summaries, visibleList]);
  ...
  return <main className={styles.page}>
    ...
    {coreReady && !coreReadOnly && <section className={styles.coreStrip}><div><CircleAlert size={18} /><span>נתוני ליבה ניתנים לעריכה מהירה כאן</span></div><label>הכנסה שלך<input value={core.mySalary || ""} inputMode="numeric" onChange={e => coreUpd("mySalary", Number(e.target.value.replace(/[^0-9]/g, "")) || 0)} /></label><label>הכנסת בת הזוג<input value={core.wifeSalary || ""} inputMode="numeric" onChange={e => coreUpd("wifeSalary", Number(e.target.value.replace(/[^0-9]/g, "")) || 0)} /></label></section>}
