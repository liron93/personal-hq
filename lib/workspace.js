...
//
// שני מצבים, ורק אחד מהם משנה התנהגות:
//   mode "personal"  - כמו היום: הכול ב-company_state לפי המשתמש. זה המצב של ה-owner, של משתמש בלי session,
//                      של משתמש שעוד לא אושר, ושל כל מצב שבו ה-RBAC עוד לא נפרס (טבלה חסרה) או שלא ניתן לברר.
//   mode "member"    - המשתמש חבר פעיל במרחב משותף שאינו שלו: מפתחות משותפים (בית חדש, כספים, ליבה)
//                      נקראים ונכתבים מ-workspace_state של המרחב, לפי היכולות שלו. מפתחות אישיים נשארים ב-company_state.
import {
...
async function fetchAccess(client, userId, timeoutMs) {
  try {
    ...
    if (ws.owner_id === userId) return personal(userId, "owner"); // ה-owner ממשיך בדיוק כמו היום
...
export function routeForKey(access, key) {
  ...
  return {
    ...
    canRead: canAccessStateKey(access.grants, key),
    canWrite: canAccessStateKey(access.grants, key, { write: true }),
  };
}
...
export function visibleCompanySlugs(access, slugs) {
  if (access?.mode !== "member") return [...slugs];
...
export function isCompanyReadOnly(access, slug) {
  ...
  if (access?.mode !== "member" || !key) return false;
  return !canAccessStateKey(access.grants, key, { write: true });
}

export function isHqVisible(access) {
  return access?.mode !== "member" || canViewHq(access.grants);
}
