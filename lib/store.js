...
export async function loadWithClient(client, key) {
  ...
  const cached = loadLocal(localKey(key, userId));
  ...
  try {
    ...
    if (data?.data) {
      saveLocal(localKey(key, userId), data.data);
      return data.data;
    }
...
export async function saveWithClient(client, key, value) {
  const userId = await currentUserId(client); // קורא session מקומי בלבד, בלי רשת
  saveLocal(localKey(key, userId), value); // תמיד קודם שומרים מקומית, בלי תלות בתוצאת הענן
