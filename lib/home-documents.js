...
// הקבצים מאוחסנים ב־bucket הפרטי `home-documents`; ההרשאות נאכפות ב־RLS.
...
export function createHomeDocuments(client, { timeoutMs = 8000 } = {}) {
  ...
  return {
    /** לכל משתמש תיקיית Storage פרטית משלו; אין תלות בטבלת workspaces. */
    async resolveWorkspaceId() {
      try {
        const { data, error } = await withTimeout(client.auth.getUser());
        const userId = data?.user?.id;
        if (error || !userId) return { ok: false, code: "forbidden" };
        return { ok: true, workspaceId: userId, ownerId: userId };
