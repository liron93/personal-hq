...
// הקבצים נשמרים תחת ה-workspace המשותף של בית חדש (<workspace_id>/...), לא בתיקייה לפי משתמש: כל מי שמורשה לבית רואה את אותם מסמכים.
...
// תמונות (השראות ורכש בבית חדש): אותם סוגים כמו ב-002, תמונות בלבד. הגודל נבדק פעמיים: גולמי (סינון סביר) ואחרי כיווץ בדפדפן.
export const IMAGE_MIME_TYPES = Object.freeze(["image/jpeg", "image/png", "image/webp", "image/heic"]);
export const IMAGE_MAX_BYTES = 10 * 1024 * 1024;
export const IMAGE_ACCEPT_ATTR = "image/*";

/** @returns {{ok:true,value:{mime:string,fileName:string,size:number}}|{ok:false,message:string}} */
export function validateImageFile(file, { maxBytes = IMAGE_MAX_BYTES } = {}) {
  if (!file || typeof file.size !== "number") return { ok: false, message: "צריך לבחור תמונה" };
  if (file.size === 0) return { ok: false, message: "התמונה ריקה" };
  const mime = resolveMime(file);
  if (!mime || !IMAGE_MIME_TYPES.includes(mime)) return { ok: false, message: "סוג הקובץ לא נתמך. אפשר JPG, PNG, WEBP או HEIC" };
  if (file.size > maxBytes) return { ok: false, message: `התמונה גדולה מ-${Math.round(maxBytes / (1024 * 1024))}MB` };
  return { ok: true, value: { mime, fileName: safeFileName(file.name), size: file.size } };
}
...
export function createHomeDocuments(client, { timeoutMs = 8000 } = {}) {
  ...
  return {
    /** המרחב המשותף של בית חדש (ולא תיקייה לפי משתמש), כדי שכל החברים בו רואים את אותם קבצים. אין כזה (או שהטבלה עוד לא קיימת) = not_enabled. */
    async resolveWorkspaceId() {
      try {
        ...
        if (error) return { ok: false, code: error.code === "PGRST205" || error.code === "42P01" ? "not_enabled" : "network" };
