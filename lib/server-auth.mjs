...
/**
 * שער אחיד לכל route: אימות -> הרשאה (API_ALLOWED_USER_IDS אם הוגדר) -> הגבלת קצב.
...
export function createApiGuard({ authenticate = authenticateRequest, env = process.env, limiter = createRateLimiter(), log = logCode } = {}) {
  return async function guard(request, { scope, limit = 30, windowMs = 60000, allowDemo = true } = {}) {
    let user = null;
    let result;
    ...
    if (!user.demo) {
      ...
      // לא הוגדר = כל משתמש מאומת מותר. הוגדר (גם אם שגוי) = רק מי שברשימה; רשימה לא תקינה נכשלת סגור.
      if (raw && !parseAllowedUserIds(raw).has(user.id)) {
        log(scope, "forbidden");
